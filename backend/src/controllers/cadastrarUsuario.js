...
const cadastrarUsuario = async (req, res) => {
  ...
  try {
    ...
    if (rowCountConsultaExisteEmail > 0) {
      return res.status(400).json({mensagem:"Dados de cadastro inválidos, por favor verifique e tente novamente."});
    }
    ...
    const { rows, rowCount } = await pool.query(queryCadastroUsuario, paramsCadastroUsuario);
